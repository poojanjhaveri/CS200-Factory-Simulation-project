 ...
 * It uses a protocol established with the Message class. 
 * It contains instances of agents and calls their methods appropriately based on received messages from clients.
 * So, this class runs a loop that constantly checks for new clients and creates a new thread for each. 
 * @author David Zhang
 *
 */
...
//	private FeederAgent feederAgent; 
	...
	/**
	 * @brief Instances Server, prompting the user for a port number and beginning the
	 * loop that checks for clients.
	...
	/**
	 * @brief Internal class that handles each new thread for a client. Creates Socket, PrintWriter, etc.
	 */
	class HandleAManager implements Runnable {
		...
		/**
		 * @brief Processes a given message and executes the proper method from an agent.
...
//			}
		...
		/**
		 * @brief Stops the loop of the server, letting the server turn off. This stops the entire program.
		 */
