...
public class PartsAgentTest extends TestCase{
	...
	assertTrue("KitRobot should have gotten two messages" + getLogs(), kitrobot.log.size()==1);
	...
	assertTrue("Nest should have gotten 16 requests" + getLogs(), nest.log.size()==8);
	assertTrue("KitsStarted size should be 2", parts.kitsStarted.size()==1);
        assertTrue("KitZero and KitOne should be false, signifying neither kits are ready", parts.kitZero==false && parts.kitOne==false);
        parts.msgEmptyKitReady(kit1);
	assertTrue("Parts should have set kit1 status as ready " , parts.kitOne==true);
        parts.pickAndExecuteAnAction();
        assertTrue("KitOne should be false " , parts.kitOne==false);
        assertTrue("KitsStarted size should be 1", parts.kitsStarted.isEmpty());
        assertTrue("Kit1needsparts size should be 8", parts.kit1NeedsParts.size()==8);
	parts.msgEmptyKitReady(kit0);
	assertTrue("kit0 status should be ready", parts.kitZero==true);
        parts.pickAndExecuteAnAction();
        assertTrue("KitZero should be false " , parts.kitZero==true);
        assertTrue("KitsStarted size should be 0", parts.kitsStarted.isEmpty());
        assertTrue("Kit0needsparts size should be 8", parts.kit0NeedsParts.isEmpty());
	for (int i = 0; i < 8; i++) {
			parts.msgHereIsPart(kit.getPart(i));
		}
	for (int i = 0; i < 8; i++) {
	...
	assertTrue("Kit0NeedsParts size should be 7", parts.kit0NeedsParts.isEmpty() && parts.kit1NeedsParts.size()==7);
	assertTrue("Grips should be 1", parts.grips.size()==1);
        assertTrue("Kit0 size should be 1", parts.kit0.parts.isEmpty());
        kits.clear();
        kits.add(kitDif);
        parts.msgHereIsKit(kits);
        assertTrue("Kits size is 2 and newKit array is 1", parts.kits==1 && parts.newKit.size()==2);
        parts.pickAndExecuteAnAction();
        parts.pickAndExecuteAnAction();
        parts.pickAndExecuteAnAction();
        assertTrue("Grips should have no parts in it", parts.grips.isEmpty());
	...
	assertTrue("KitRobot should have recieved msgKitIsFull"+ getLogs(), kitrobot.log.containsString("msgKitIsFull") && kitrobot.log.size()==2);
	parts.pickAndExecuteAnAction();
        assertTrue("The new kit should have moved to kitsStarted array", parts.kitsStarted.size()==1);
        }
        
        public void testNonNorm(){
            kit = new Kit("Test Kit");
       
        kit1= new Kit("Kit 1");
        
        missing = new ArrayList<Part>();
        kit1.standNum=Kit.StandNum.one;
        
        for (int i = 0; i < 3; i++) {
            missing.add(new Part(i+4));
            kit.addPart(new Part(i));
            kit.getPart(i).setNestNum(i);
        }
        kits = new ArrayList<Kit>();
	...
	parts.msgPartsMissing(missing, kit);
        assertTrue("Kit1NeedsParts size is 3", parts.kit1NeedsParts.size()==3);
        assertTrue("Nest got 3 messages for new parts" + getLogs(), nest.log.size()==3);
        assertTrue("Nest got msgNeedParts" + getLogs(), nest.log.containsString("msgNeedPart"));
        parts.msgHereIsPart(missing.get(0));
        parts.msgHereIsPart(missing.get(1));
        parts.msgHereIsPart(missing.get(2));
        assertTrue("Inventory size is 3", parts.inventory.size()==3);
        parts.pickAndExecuteAnAction();
        parts.pickAndExecuteAnAction();
        parts.pickAndExecuteAnAction();
        assertTrue("Kit1NeedsParts size is 0", parts.kit1NeedsParts.isEmpty());
